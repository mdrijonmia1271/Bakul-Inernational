...
class DealerSaleEditCtrl extends Admin_Controller {
    ...
    private function change(){
        ...
        $data = array(
            ...
            'total_return'      => $this->input->post('total_new_return'),
            'due'               => $due
        );
        ...
        if(!empty($_POST['return_product'])){
            foreach ($_POST['return_product'] as $key => $value) {
                ...
                $data = [
                            ...
                            'quantity'              => abs($_POST['return_quantity'][$key]),
                            'return_quantity_kg'    =>  $_POST['return_quantity_kg'][$key],
                        ];

                if (save_data('sapitems', $data,['id' => $_POST['return_id'][$key]])) {
    ...
    private function handelReturnStock($index)
    {
        ...
        $where['godown_code'] = $_POST['return_godown_code'][$index];
        ...
        if (!empty($record)) {
            ...
            // calculate quantity
            $newQuantity = $_POST['return_quantity'][$index] - $_POST['return_old_quantity'][$index];
