    ...
    private function search()
    {
        ...
        if (!empty($result)) {
            foreach ($result as $_key => $item) {
                ...
                if ($item->sap_type == 'cash') {
                    $cashInfo = json_decode($item->address);
                    $data[$_key]['name'] = filter($item->party_code);
                    $data[$_key]['address'] = $cashInfo->address;
                }else{
                    $clientInfo = get_row('parties', ['code' => $item->party_code, 'trash' => 0], ['name', 'address']);
                    $data[$_key]['name'] = $clientInfo->name;
                    $data[$_key]['address'] = $clientInfo->address;
                }
