...
class Sales_report extends Admin_Controller
{
    ...
    public function sales_report_item()
    {
        ...
        $items                = ['sapitems.sap_at', 'SUM(sapitems.quantity) AS quantity', 'AVG(sapitems.sale_price) AS sale_price', 'products.product_code', 'products.product_name', 'products.product_model', 'products.unit', 'godowns.name as godown_name'];
  ...
  public function clientwise_item_report()
    {
        $this->data['subMenu'] = 'data-target="clientwise_item_report"';

        // get all product
        $this->data['allProduct'] = get_result('products', ['status' => 'available'], ['product_code', 'product_model']);
        
        // get all client
        $this->data['allClient'] = get_result('parties', ['type' => 'client','status' => 'active'], ['name', 'mobile','address','code']);
       ...
       /* // get all category
        ...
        // Today's Data
        $where = ['sapitems.status' => 'sale', 'sapitems.trash' => 0,'saprecords.trash' => 0];
        $groupBy = "products.product_code";
        if (isset($_POST['show'])) {
        ...
        $tableTo              = ['saprecords','products','godowns'];
        $joinCond             = ["sapitems.voucher_no=saprecords.voucher_no","sapitems.product_code=products.product_code", "godowns.code=sapitems.godown_code"];
        $items                = ['sapitems.sap_at','SUM(sapitems.quantity) AS quantity','products.product_model', 'products.unit', 'products.subcategory', 'products.brand', 'godowns.name as godown_name'];
        $this->data['results'] = get_join('sapitems', $tableTo, $joinCond, $where, $items, $groupBy);
        ...
        $this->load->view($this->data['privilege'] . '/includes/headermenu', $this->data);
        $this->load->view('components/report/sales_nav', $this->data);
        $this->load->view('components/report/clientwise_item_report', $this->data);
        $this->load->view($this->data['privilege'] . '/includes/footer', $this->data);
