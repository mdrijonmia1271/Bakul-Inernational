...
class Dashboard extends Admin_controller{
    ...
    public function index() {
        ...
        // Supplier paid
        $where = [];
        $where = array('transaction_at' => $today, 'trash' => 0, 'transaction_by' => 'supplier','comment !=' =>'comission','godown_code' => $godown_code);
       ...
       //retail commitment
        $where_retail_customer['commitment_date'] = date('Y-m-d');
        $where_retail_customer['status'] = 'sale';
        $where_retail_customer['sap_type'] = 'cash';
        $where_retail_customer['due !='] = 0;
        $where_retail_customer['trash'] = 0;
        
    
        $allRetailSale = $this->action->read('saprecords',$where_retail_customer);
     
        
        foreach($allRetailSale as $key=> $value){
       		
   			if($value->commitment_date != '0000-00-00'){
   				$this->data['irregularInstallment'][$key]['party_code'] = $value->party_code;
   				$this->data['irregularInstallment'][$key]['voucher_no'] = $value->voucher_no;
   				$this->data['irregularInstallment'][$key]['total_bill'] = $value->total_bill;
   				$this->data['irregularInstallment'][$key]['paid'] =  $value->paid;
   				$this->data['irregularInstallment'][$key]['due'] = $value->total_bill - $value->paid;
   			}
       ...
       foreach($supplierDueInfo as $key=>$value){
        ...
        $this->data['complainInfo'] = get_result("complain");
