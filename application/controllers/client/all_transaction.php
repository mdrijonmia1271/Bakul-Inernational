...
class All_transaction extends Admin_Controller {
    ...
    public function index() {
       ...
       // get all transaction and search
        ...
        $select= ['partytransaction.id','partytransaction.transaction_at', 'partytransaction.inc_code', 'partytransaction.party_code', 'partytransaction.transaction_via', 'partytransaction.credit', 'parties.name'];
    ...
    public function view($id=null) {
        ...
        if($this->data['records']->initial_balance >= 0){
            $balance =($this->data['records']->initial_balance + $debit) - ($credit + $remission);
            $totalPaid = $credit + $remission;
        }else{
            $balance = $debit - ($credit + $remission + abs($this->data['records']->initial_balance));
            $totalPaid = $credit + $remission + abs($this->data['records']->initial_balance);
        }
        
     
        $this->data['previousBalance'] = $balance + $this->data['records']->credit + $this->data['records']->remission;
        $this->data['totalPaid'] = $totalPaid;
        $this->data['currentBalance'] = $balance;
