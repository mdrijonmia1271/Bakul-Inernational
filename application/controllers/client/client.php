...
class Client extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_POST['add'])) {
            ...
            $code = $counter;
            if ($_POST['customer_type'] == 'dealer'){
                $client_code = $gPrefix . 'D' . $code;
            }elseif($_POST['customer_type'] == 'sub_dealer'){
                $client_code = $gPrefix . 'S' . $code;
            }else{
                $client_code = $gPrefix . 'U' . $code;
            }
            ...
            $data             = array(
                ...
                'type'               => "client",
                'customer_type'     => $this->input->post('customer_type'),
                'reference_dealer'  => ($this->input->post('customer_type') == 'sub_dealer' ? $this->input->post('reference_dealer') : null), 
            ...
            if ($status == false){
                ...
                $code_ = get_voucher($id, 6);
                
                $options = array(
                    'title' => 'success',
                    'emit'  => 'Customer Successfully Saved. Your Customer ID is : ' . '<b>' . $code_ . '</b>',
                    'btn'   => true
                );
                save_data("parties", ['code'=>$code_], ['id'=>$id]);
        ...
        $this->data['dealers'] = get_result('parties', ['customer_type'=>'dealer']);
    ...
    public function onscroll_load_all_data()
    {
        ...
        if (!empty($_POST['find'])) {
            ...
            if (!empty($_POST['customer_type'])) {
                if ($_POST['customer_type'] != 'type_null') {
        ...
        if ($total_row > 0) {
            foreach ($getData as $member) {
                ...
                $data[$i]['type']         = $type;
                $data[$i]['customer_type']= $member->customer_type;
    ...
    public function preview()
    {
        ...
        $this->data['partyInfo'] = get_row_join("parties", 'godowns', 'godowns.code=parties.godown_code', ['parties.code' => $_GET['partyCode'], 'parties.trash'=>0], ['parties.*', 'godowns.name as godown_name']);
    ...
    public function edit()
    {
        ...
        if (isset($_POST['update'])) {
            ...
            if ($photo != NULL) {
                $data = array(
                    ...
                    'guarantor_address2' => $this->input->post('guarantor_address2'),
                    'customer_type'      => $this->input->post('customer_type'),
                    'reference_dealer'   => $this->input->post('reference_dealer'), 
        ...
        $this->data['info']     = $this->action->read("parties", array('code' => $code, 'trash'=>0));
        $this->data['dealers']  = $this->action->read("parties", array('customer_type' => 'dealer'));
