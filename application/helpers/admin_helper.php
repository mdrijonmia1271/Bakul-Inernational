...
// get Date Day Month Year (25-06-2022) Wise
if (!function_exists('d_formate')) {
    function d_formate($value = null, $formate = 'd-m-Y') {
        if (!empty($value)) {
...
if (!function_exists('get_voucher')) {
    function get_voucher($id, $digite = 6, $prefix = null)
    {
        if (!empty($id)) {
            if (!empty($prefix)) {
                return $prefix . date('ymd') . str_pad($id, $digite, 0, STR_PAD_LEFT);
            } else {
                return date('ymd') . str_pad($id, $digite, 0, STR_PAD_LEFT);
            }
        }
        return false;
    }
...
if (!function_exists('get_code_table')) {
    function get_code_table($table, $where = [], $digite = 3, $prefix = null)
    {
        ...
        if (!empty($table)) {
            ...
            if (!empty($prefix)) {
                return $prefix . str_pad(++$total_row, $digite, 0, STR_PAD_LEFT);
            } else {
                return str_pad(++$total_row, $digite, 0, STR_PAD_LEFT);
            }
        }
        return false;
    }
...
// get cash client
if (!function_exists('get_party_balance')) {
    function get_party_balance($party_code = null)
    {
        //get main CodeIgniter object
        $ci =& get_instance();
        ...
        if (!empty($party_code)) {
            ...
            // get party info
            $info = $ci->db->query("SELECT parties.code, parties.name, parties.initial_balance, partytransaction.debit, partytransaction.credit , partytransaction.comission , partytransaction.remission FROM( SELECT code, name, initial_balance FROM parties WHERE code='$party_code' AND trash=0)parties LEFT JOIN ( SELECT party_code, sum(debit) AS debit, sum(comission) AS comission, SUM(credit) AS credit, SUM(remission) AS remission FROM partytransaction WHERE trash=0 GROUP BY party_code )partytransaction ON parties.code=partytransaction.party_code")->row();
            ...
            $comission        = (!empty($info->comission) ? $info->comission : 0);
            $remission        = (!empty($info->remission) ? $info->remission : 0);

            // get balance
            $balance = ($debit + $comission) - ($credit + $remission) + $initital_balance;
