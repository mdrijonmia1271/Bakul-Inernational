...
<div class="container-fluid" ng-controller="clientProfitReportCtrl">
    ...
    // where section
    $where = array();
    $where_comission = array();
    $where['trash'] = 0;
    $where_comission['trash'] = 0;
    $where_comission['transaction_by'] = 'supplier';
    $where_comission['transaction_via'] = 'commission';
    
    if (!empty($_POST['godown_code'])) {
        if ($_POST['godown_code'] != 'all') {
            $where['godown_code'] = $_POST['godown_code'];
            $where_comission['godown_code'] = $_POST['godown_code'];
        }    
    } else {
        $where['godown_code'] = $this->data['branch'];
        $where_comission['godown_code'] = $this->data['branch'];
    }    
    
    if (!empty($_POST['date']['from']) && !empty($_POST['date']['to'])) {
         $where_comission['transaction_at >='] = $_POST['date']['from'];
         $where_comission['transaction_at <='] = $_POST['date']['to'];
    }
    ...
    // calculate cost
    $total_cost = $this->action->read_sum('cost','amount',$where);
    $total_godown_cost = 0;
    if (!empty($total_cost)) {
        $total_godown_cost = $total_cost[0]->amount;
    } 
    
    // calculate comission
    $total_comission = 0;
    $all_comission = $this->action->read_sum('partytransaction','debit',$where_comission);
    if (!empty($all_comission)) {
        $total_comission = $all_comission[0]->debit;
    } 
    
    // calculate total net profit loss
    $net_profit_loss = ($total_profit_loss + $total_comission) - $total_godown_cost;

    // return profit check
    $return_profit = 0;
    if (isset($resultInfo['return_info']) && !empty($resultInfo['return_info'])) {
        $return_profit = ($resultInfo['return_info']->return_amount - $resultInfo['return_info']->purchase_amount);
    }
?>

<h4 class="text-center">
    <b>Cost: <?php echo $total_godown_cost ?? 0; ?></b>
    <br>
    <b>Comission: <?php echo $total_comission ?? 0; ?></b>
    <br>
    <b>Return Profit: <?php echo $return_profit ?? 0; ?></b>
    <br>
    <b>Total Profit: <?php echo ($total_profit_loss - $return_profit); ?></b>
    <br>
    <b>Net Profit: <?php echo ($net_profit_loss - $return_profit); ?></b>
</h4>
