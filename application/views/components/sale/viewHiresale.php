...
<style>
    ...
    ._tbl tr th, ._tbl tr td { vertical-align: text-top; padding: 1px 10px 1px 0; }
</style>
    ...
    <?php $info = $this->action->read("sapmeta", array("meta_key" => "sale_by", "voucher_no" => $result->voucher_no)); ?>
                    ...
                    <div class="col-xs-6 print-font">
                        <table class="_tbl">
                            ...
                            <tr>
                                <th>Print Time</th>
                                <th>:</th>
                                <td><?php echo d_formate($result->sap_at); ?> - <?php echo date("h:i:s A"); ?></td>
                            </tr>
                            <tr>
                ...
                <table class="table table-bordered">
                    ...
                    <?php
                        ...
                        $where     = array('voucher_no' => $result->voucher_no,'status'=>'sale');
                    ...
                    <?php
                        $total_sub_return = $total_discount = $subtotal_return = 0.0;
                        $where     = array('voucher_no' => $result->voucher_no,'status'=>'sale_purchase');
                        $saleInfo  = get_result('sapitems', $where, ['product_code', 'product_serial', 'quantity','purchase_price','sale_price', 'discount', 'godown_code']);
                        foreach ($saleInfo as $key => $row) {
                            $productInfo = get_row('stock', ['code' => $row->product_code, 'godown_code' => $row->godown_code], ['subcategory', 'category', 'product_model']);
                    ?>
                        <tr>
                            ...
                            <td><?php echo check_null($row->product_serial); ?>&nbsp;</td>
                            <td class="text-right"><?php echo $row->quantity; ?>&nbsp;</td>
                            <td class="text-left">&nbsp; <?php echo $row->purchase_price; ?></td>
                            <td class="text-right">
                                <?php
                                    $subtotal_return       = $row->purchase_price * $row->quantity;
                                    $total_sub_return      += $subtotal_return;
                                    $total_discount += $row->discount;
                                    echo f_number(($subtotal_return - $row->discount), 2)
                                ?>&nbsp;
                            </td>
                        </tr>
                    <?php } ?>
                    ...
                    <tr>
                        ...
                        <th>HMRP Value</th>
                        <td class="text-right"><?php echo f_number(($total_sub - $total_sub_return), 2); ?> &nbsp;</td>
                    </tr>
                    <tr>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered __table">
                        ...
                        for ($i = 0; $i < $result->installment_no; $i++) {
                            ...
                            <tr>
                                <td><?php echo $i + 1; ?></td>
                                <td><?php echo d_formate($next_date); ?></td>
                                <td class="text-right"></td>
                                <td class="text-right">
                                    <?php echo f_number($result->installment_amount, 2);
